...
public class AuthenticationControllerTest {
    ...
    @MockBean
    private  UserOtherDetailsManager userOtherDetailsService;
    @MockBean
    ...
    void setup() {
        registerRequest = RegisterRequest.builder()
                ...
                .password("111111aA*")
                .gender(Gender.MALE)
                .dateOfBirth(Date.from(LocalDate.of(1997, 2, 19).atStartOfDay(ZoneId.systemDefault()).toInstant()))
                .build();
    ...
    void registerShouldPass() throws Exception {
        Mockito.when(authenticationManager.register(Mockito.any(RegisterRequest.class))).thenReturn(new AuthenticationResponse("some-token"));
    ...
    void registerShouldFailWithWrongPasswordFormat() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/auth/register")
                        ...
                        .content(objectMapper.writeValueAsBytes(registerRequest)))
                .andExpect(status().isBadRequest())
                .andExpect(jsonPath("$.password").value("Password must consists of 1 uppercase letter, 1 lowercase letter, 1 numeric character and at least 8 characters long"));

    }

    @Test
    void authenticationShouldSuccess() throws Exception {
        Mockito.when(authenticationManager.authenticate(Mockito.any(AuthenticationRequest.class))).thenReturn(new AuthenticationResponse("some-token"));
    ...
    void authenticateShouldFailWithNonExistingAccount() throws Exception {
        AuthenticationRequest newAuthenticationRequest = AuthenticationRequest.builder()
                ...
                .password("111111aA*")
                .build();
        Mockito.when(authenticationManager.authenticate(Mockito.any(AuthenticationRequest.class))).thenThrow(new AuthenticationException("asd") {
        });
        mockMvc.perform(post("/api/v1/auth/authenticate")
                        .contentType(MediaType.APPLICATION_JSON)
