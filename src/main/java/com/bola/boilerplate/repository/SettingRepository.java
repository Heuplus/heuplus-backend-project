...
public interface SettingRepository extends JpaRepository<Setting, UUID> {
  ...
  */
  @Query(
      "SELECT new com.bola.boilerplate.dto.SettingDto(id, key, value) FROM Setting WHERE id ="
          + " :settingId AND user.id = :userId")
  Optional<SettingDto> findSettingDtoByIdAndUser(
      @Param("settingId") UUID settingId, @Param("userId") UUID userId);

  /*
   Implementation blueprint for getting list of setting for a user without relations
  */
  @Query(
      "SELECT new com.bola.boilerplate.dto.SettingDto(id, key, value) FROM Setting WHERE user.id ="
          + " :userId")
